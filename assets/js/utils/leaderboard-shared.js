 ...
 * Uses global Formatters (formatters.js) when available.
 */
(function (global) {
  'use strict';
  ...
  /** Leaderboard / row label for team comp type: TH, TT (best total), TW, TD. */
  function formatTeamCompMnemonicForLeaderboard(teamRule) {
    if (teamRule === 'total') return 'TT';
  ...
  // Preserve BGS's existing per-competition leaderboard cap.
  var MAX_PLACES = 20;
  var MAX_OVERALL_OUTINGS = 99;
  ...
  function parseSocietyOverallStatus(statusStr) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (p === 'OAPV') {
        ...
        overallBestN = Math.min(MAX_OVERALL_OUTINGS, Math.max(0, parseInt(p.slice(4), 10) || 0));
      }
    }
  ...
  function parseNHolesToken(t) {
    ...
    if (!m) return null;
    var inspected = inspectNHolesHolesFromText(m[1].replace(/-/g, ' '));
    if (!inspected.holes.length || inspected.invalid.length || inspected.duplicates.length) return null;
    return {
      holes: inspected.holes,
      usePoints: m[2] === 'p',
